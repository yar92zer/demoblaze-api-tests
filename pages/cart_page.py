"""Корзина: таблица позиций, итог, переход к оформлению заказа."""
...
class CartPage(BasePage):
    ...
    Таблицу рисует `js/cart.js` и делает это в два шага: сначала
    `POST /viewcart` отдаёт позиции, потом на **каждую** позицию уходит
    отдельный `POST /view` за названием и ценой, и строка добавляется уже
    в его коллбэке. Отсюда два следствия:

    - ждать надо не появления строк, а совпадения их числа с числом
      позиций в ответе `/viewcart` — иначе прочитаем половину таблицы;
    - порядок строк не определён: чей `/view` вернулся раньше, тот и выше.
      Сравнивать состав корзины нужно множествами, а не списками.

    Итог в `#totalp` накапливается по мере прихода строк, поэтому читать
    его до окончания отрисовки бессмысленно.
    """
